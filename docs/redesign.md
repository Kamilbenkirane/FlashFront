...
Browser checks intercept API calls with synthetic data and save screenshots to the ignored `artifacts/` directory. Screens and their empty, error, and recovery states were visually checked at 320, 390, and 1280 pixels. Authentication and onboarding cover 48 screenshots; study checks cover reveal-before-rating, session completion and resuming, visible review age and recall streak, long math content, assistant prompts, and reduced motion. TypeScript, Biome, and all 40 unit tests pass. The final study screen was also checked in the iPhone simulator.

[TestFlight build 1.2.30](https://github.com/Kamilbenkirane/FlashFront/actions/runs/33977785417) was built from `8c76817`. Apple reports it as `VALID` and `IN_BETA_TESTING` for internal testers. Its signed IPA identifies itself as Shuffle and contains opaque 1024px default, dark, and tinted icons for both iPhone and iPad. The compiled launcher image and Apple's processed TestFlight and App Store icons were visually checked. Apple accepted the listing name **Shuffle: Flashcards**, since **Shuffle** alone was unavailable. The app and Home Screen label remain **Shuffle**. These checks verify the release assets; installation on the user's physical phone was not directly observed.
