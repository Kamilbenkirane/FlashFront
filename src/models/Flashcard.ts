...
class Flashcard implements IFlashcard {
    ...
    constructor(dictCard: any) { // Initializes a new Flashcard object with data from dictCard
        ...
        this.lastReviewTimestamp = dictCard.last_review_timestamp ? moment(dictCard.last_review_timestamp) : null;
    ...
    updateSecondsSinceLastReview(): void {
        ...
        if (this.lastReviewTimestamp) {
            this.secondsSinceLastReview = moment().diff(this.lastReviewTimestamp, 'seconds');
        } else {
            this.secondsSinceLastReview = undefined; // Reset or keep undefined if there's no lastReviewTimestamp
        }
    }
    updateStreak(success: boolean): void { // Updates the streak based on review outcome
        if (success) {
    ...
    calculatePopupScore(): number { // Calculates the score determining the card's review priority
        let popupScore = 0;
        this.updateSecondsSinceLastReview();
        // If the card has never been reviewed, start with a base score
        if (!this.lastReviewTimestamp) {
            popupScore = 2 + this.malus;
        } else if (this.streak === 0) { // If the streak is 0, use a slightly lower base score
            ...
            if (this.secondsSinceLastReview < Tmax) {
                popupScore = Math.min(10000, Math.exp(0.01 * (this.secondsSinceLastReview - Tmax))) + this.malus;
            } else {
    ...
    updateCard(success: boolean): void { // Updates the card with the result of the latest review
