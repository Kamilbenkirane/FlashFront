...
class Session {
    ...
    constructor(sessionData: any[]) { // Consider a more specific type for sessionData if possible
        this.pile = sessionData ? sessionData.map(data => new Flashcard(data)) : [] ;
        this.flashcards = [];
        this.updateActiveCardsFromPile();
        this.totalCards = this.flashcards.length;
        this.totalScore = this.flashcards.reduce((acc, card) => acc + card.popupScore, 0);
    }

    updateActiveCardsFromPile(): void {
        const previouslyReviewedCards = this.pile.filter(card => card.lastReviewTimestamp !== null);
    ...
    getNextCard(): Flashcard | undefined {
        // console.log('Getting next cardxxx');
        //console.log('Active cards:', this);
        // Update active cards from the pile
        this.updateActiveCardsFromPile();
